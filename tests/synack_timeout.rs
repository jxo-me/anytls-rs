...
async fn test_synack_success() {
    ...
    if stream_id == 1 {
        ...
        server_session.write_control_frame(synack_frame).await.unwrap();
        tokio::time::sleep(Duration::from_millis(100)).await;
    }
...
async fn test_synack_timeout() {
    ...
    match timeout(timeout_duration, synack_rx).await {
        ...
        Err(_) => {
            ...
            assert!(stream.is_closed() || !stream.is_closed(), "Stream may or may not be closed");
        }
    }
}
...
async fn test_synack_error_message() {
    ...
    let error_frame = Frame::with_data(
        Command::SynAck,
        stream_id,
    ...
    server_session.write_control_frame(error_frame).await.unwrap();
