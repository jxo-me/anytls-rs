...
pub async fn send_authentication<W: AsyncWriteExt + Unpin>(
    ...
    let padding_len = if padding_len < 0 { 0 } else { padding_len as u16 };
...
mod tests {
    ...
    async fn test_authentication_success() {
        ...
        send_authentication(&mut client, &password_hash, &padding).await.unwrap();
        
        // Server authenticates
        authenticate_client(&mut server, &password_hash, &padding).await.unwrap();
    }
    ...
    async fn test_authentication_failure() {
        ...
        assert!(matches!(result.unwrap_err(), AnyTlsError::AuthenticationFailed));
    }
